...
let numberm = 20;
let numberm2 = 0;
mnumber.textContent = `💯 Remaining attempts: ${numberm}`;
recordt.textContent = `🏆 Record: ${numberm2}`;
check.addEventListener("click", (e) => {
  e.preventDefault();
  let numinp = Number(numinp1.value);
  let cheked = numinp === secretNum;
  if (!numinp) {
    startGameText.textContent = "⛔ The number was not entered correctly";
  } else if (cheked) {
    ...
    startGameText.textContent = "📉 The secret number is lower";
    numberm--;
    mnumber.textContent = `💯 Remaining attempts: ${numberm}`;
        if (numberm === 0) {
        startGameText.textContent = "❌ You lost the game";
        secretBox.textContent = secretNum;
      check.classList.add("hidden");
    ...
    startGameText.textContent = "📈 The secret number is higher";
    numberm--;
    mnumber.textContent = `💯 Remaining attempts: ${numberm}`;
    if (numberm === 0) {
      startGameText.textContent = "❌ You lost the game";
